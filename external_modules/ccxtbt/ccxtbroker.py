...
def get_extra_params_for_order(order):
    params = {}
    if order['info']['order_status'] == 'Untriggered':
        params['stop_order_id'] = order['id']
    return params
...
class CCXTBroker(with_metaclass(MetaCCXTBroker, BrokerBase)):
    ...
    def cancel(self, order):
        ...
        if ccxt_order[self.mappings['canceled_order']['key']] != self.mappings['canceled_order']['value']:
            extra_params = get_extra_params_for_order(ccxt_order)
            self.store.cancel_order(oID, order.data.p.dataname, extra_params)
            # check again order status
            ccxt_order = self.store.fetch_order(oID, order.data.p.dataname)
